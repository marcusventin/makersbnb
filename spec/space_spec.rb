...
describe Space do
  describe '.add' do
    it 'add space to the database' do
      test_space = Space.add(name: 'test_property', description: 'test_description', ppn: 100, start_date: Date.parse(Date.today), end_date: Date.parse(Date.today+1))
  ...
  describe '.all' do
    it 'displays all spaces' do
      setup_test_database
    test_space = Space.add(name: 'test_property', description: 'test_description', ppn: 100, start_date: Date.parse(Date.today), end_date: Date.parse(Date.today+1))
    Space.add(name:'beach house', description:'on beach', ppn: 400, start_date: Date.parse(Date.today), end_date: Date.parse(Date.today+1))
